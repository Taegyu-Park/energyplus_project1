"""
Figure 17: Monthly HVAC Self-Sufficiency Rate (SSR) and BIPV-to-HVAC Direct Supply Comparison
...
def main():
    ...
    # 2. Line Chart (Right Y-axis) - Self-Sufficiency Rate (SSR) in %
    l_c2, = ax_line.plot(x, c2_opt_pv['SSR_%'], color=color_c2_line, linewidth=2.5, marker='o', markersize=6, label='Case 2 (Opt BIPV) SSR')
    l_c3, = ax_line.plot(x, c3_monthly['SSR_%'], color=color_c3_line, linewidth=2.5, marker='s', markersize=6, label='Case 3 (Kinetic BIPV) SSR')
    
    # Titles and labels
    ax_bar.set_ylabel("Monthly BIPV-to-HVAC Direct Supply [MWh]", fontweight='bold', fontsize=12)
    ax_line.set_ylabel("Self-Sufficiency Rate (SSR) [%]", fontweight='bold', fontsize=12)
    ax_bar.set_xlabel("Month", fontweight='bold', fontsize=12)
    ax_bar.set_title("Monthly BIPV-to-HVAC Direct Supply & Self-Sufficiency Rate (SSR)", fontsize=15, fontweight='bold', pad=25)
    ...
    # Annotate SSR line values (prevent overlap using smart positioning)
    for tx, val2, val3 in zip(x, c2_opt_pv['SSR_%'], c3_monthly['SSR_%']):
    ...
    ax_line.legend(handles=[l_c2, l_c3], labels=['Case 2 SSR', 'Case 3 SSR'], 
    ...
    # Output file paths
    figure_dir = os.path.normpath(os.path.join(script_dir, "..", "plot", "fig17_hvac_self_sufficiency_opt_pv"))
    os.makedirs(figure_dir, exist_ok=True)
    output_png = os.path.join(figure_dir, "hvac_self_sufficiency_comparison_opt_pv.png")
    output_svg = os.path.join(figure_dir, "hvac_self_sufficiency_comparison_opt_pv.svg")
    ...
    print(f"\nSuccessfully generated SSR & Direct Supply comparison plot (Max PV Angle, Single Axis):")
