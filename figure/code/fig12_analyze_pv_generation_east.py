...
from the simulation database (eplusout.sql) in case3_v3_east.
It sums the values across all BIPV angles, converts Joules to kWh,
...
def main():
    ...
    db_path = os.path.normpath(os.path.join(
        project_root, "case_analysis", "bipv_variation", "5zone_NSEW_korean", "case3_v3_east", "eplusout.sql"
    ))
    ...
    # Also save with alias filenames
    alias_png_path = os.path.join(figure_dir, "pv_annual_generation_heatmap_east.png")
    alias_svg_path = os.path.join(figure_dir, "pv_annual_generation_heatmap_east.svg")
    ...
    print("Querying and aggregating East PV generation data...")
    c.execute(query)
    rows = c.fetchall()
    conn.close()
    ...
    print("\n" + "="*80)
    print(" EAST WALL ANNUAL PV GENERATION MATRIX (6 rows x 9 columns) - Unit: kWh ")
    print("="*80)
    ...
    print(f"Total Annual East PV Generation: {total_generation_kwh:.2f} kWh ({total_generation_kwh/1000:.3f} MWh)")
    ...
    ax.set_title("Annual East Wall BIPV Panel Generation Heatmap (kWh)", fontsize=15, fontweight="bold", pad=15)
    ...
    print(f"Saved East Heatmap plots to:\n  - {plot_png_path}\n  - {plot_svg_path}")
