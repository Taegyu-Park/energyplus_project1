...
class KineticBIPVPlugin(EnergyPlusPlugin):
    ...
    def on_begin_zone_timestep_before_set_current_weather(self, state) -> int:
        ...
        # 3. 야간 제어: 셰이딩 비활성화 및 BIPV 차양/발전기 off
        if sun_alt <= 0.0:
            self._override_shading_and_bipv(state, optimal_angle=0, shading_on=False)
        ...
        # 4. 주간 제어: 태양 고도 정면 추적 최적각 계산
        target_angle = sun_alt
        best_angle = min(ANGLES, key=lambda x: abs(x - target_angle))
 
        # 5. WindowShadingControl 및 BIPV 10개 각도 투과율/가용성 통합 제어
        self._override_shading_and_bipv(state, optimal_angle=best_angle, shading_on=True)
    ...
    def _get_handles(self, state):
        ...
        # A. WindowShadingControl 연동 스케줄 액추에이터 취득
        self.handles["shading_status"] = self.api.exchange.get_actuator_handle(
            state, "Schedule:Constant", "Schedule Value", "ShadingStatusSched"
        )
        self.handles["slat_angle"] = self.api.exchange.get_actuator_handle(
            state, "Schedule:Constant", "Schedule Value", "SlatAngleSched"
        )
 
        # B. 10개 각도의 개별 차양 투과율 & 가용성 액추에이터 취득
        ...
        for key, val in self.handles.items():
            ...
            if isinstance(val, dict):
                ...
                print(f"DEBUG_WARN: FAILED TO GET HANDLE for sensor/actuator: {key}", flush=True)
                any_failed = True
    ...
    def _override_shading_and_bipv(self, state, optimal_angle, shading_on):
        # 1. 창문 슬랫 차양 제어
        status_val = 1.0 if shading_on else 0.0
        self.api.exchange.set_actuator_value(state, self.handles["shading_status"], status_val)
        self.api.exchange.set_actuator_value(state, self.handles["slat_angle"], float(optimal_angle))
        
        # 2. 개별 차양 표면 투과율 및 발전기 가용성 제어
        for angle in ANGLES:
            trans_act = self.handles["trans_actuators"][angle]
            avail_act = self.handles["avail_actuators"][angle]
            
            if angle == optimal_angle and shading_on:
